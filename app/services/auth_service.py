...
def register_user(name: str, email: str, password: str) -> dict:
    with get_tx() as conn:
        existing = users_repo.get_user_by_email(conn, email)
        if existing:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Email already registered")

        hashed = bcrypt.hash(password)
        user = users_repo.insert_user(conn, name, email, hashed)
...
def verify_account_by_token(token: str) -> dict:
    ...
    with get_tx() as conn:
        ...
        users_repo.mark_verified(conn, user_id)
        return {"ok": True}
...
def get_auth_status(user_id: int) -> dict:
    with get_tx() as conn:
        user = users_repo.get_user_by_id(conn, user_id)
